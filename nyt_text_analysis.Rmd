...
tidied_text <- cleaned_articles_df %>% 
  select(pub_date, 
         lead_paragraph,
         subsection_name) %>% 
  tibble(line = 1:464, text = lead_paragraph) %>%  #change this too
...
After creating a `tf-idf` score, the plots identify word that are important to a text but not used *too* commonly. I have further gone in and grouped these by continents. As we can see, the "important words" by `tf-idf` are actually countries or regions within the continents. That being said, we still see some interesting phrases such as how the Americas includes "president", "border", "trump," and Australia includes "gangs" and "detention".
...
sentiment_text_lead <- cleaned_articles_df %>% 
  select(pub_date, 
         lead_paragraph, 
         subsection_name) %>% 
  tibble(line = 1:464, text = lead_paragraph) %>%  # this needs to be fixed; the erro keeps popping up in knitting
  unnest_tokens(word, lead_paragraph) %>% 
  select(!text) %>% 
  anti_join(stop_words)
...
```
```{r sentiment plots, fig.show='hold', out.width="50%"}
# Using bing dictionary: What does the net positive and negative look like for each continent?
sentiment_text_lead %>% 
